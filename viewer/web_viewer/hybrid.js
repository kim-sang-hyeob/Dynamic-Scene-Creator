...
async function main() {
  ...
  const downsample = 1; // Full quality rendering
  ...
  window.addEventListener(
    ...
    (e) => {
      ...
      if (e.shiftKey) {
        ...
        // Default wheel = zoom (dolly in/out)
        let d = 4;
  ...
  canvas.addEventListener("contextmenu", (e) => {
    e.preventDefault(); // Prevent context menu to enable right-click pan
  });
  ...
  canvas.addEventListener("mousemove", (e) => {
    ...
    if (down == 1) {
      ...
      let dy = (8 * (e.clientY - startY)) / innerHeight;
      let d = 4;
